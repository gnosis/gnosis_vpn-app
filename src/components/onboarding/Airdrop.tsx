...
export default function Airdrop({ setStep }: { setStep: (step: string) => void }) {
  ...
  const fundingTool = createMemo(() => {
    ...
    return rm && typeof rm === "object" && "PreparingSafe" in rm ? rm.PreparingSafe.funding_tool : undefined;
  });
  ...
  return (
    <div class="h-full w-full flex flex-col items-center p-6">
          ...
          <input
            ...
            onKeyDown={e => {
              if ((e as KeyboardEvent).key === "Enter") {
                e.preventDefault();
                const canClaim =
                  secretCode().length > 0 && !loading() && !claimed() && fundingTool() !== "CompletedSuccess";
                if (canClaim) {
                  void handleClaim();
                }
              }
      ...
      <Show when={fundingTool() === "CompletedSuccess"}>
        <Button size="lg" onClick={() => appActions.setScreen("synchronization")}>
          Proceed
        </Button>
      </Show>
    </div>
