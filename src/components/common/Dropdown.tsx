...
export function Dropdown<T>(props: DropdownProps<T>) {
  ...
  return (
    <>
      <div
        ref={root}
        class={`relative text-left ${props.size === "sm" ? "bg-black white pr-0 pl-4 py-2 rounded-full min-w-48" : "w-full"} flex flex-row gap-2 items-center ${props.class ?? ""}`}
      >
        ...
        <button
          ...
          class={`inline-flex items-center justify-center rounded-2xl px-4 py-2
               ...
               transition-transform duration-150 ease-out select-none ${props.size === "sm" ? "h-6 w-10" : "h-10 w-16"}`}
        ...
        >
          <img
            src={arrowDownIcon}
            class={`opacity-90 ${props.size === "sm" ? "w-4 h-4" : "w-5 h-5"} transition-transform duration-200 ${open() ? "rotate-180" : ""}`}
            alt="dropdown"
            aria-hidden="true"
          />
        </button>
