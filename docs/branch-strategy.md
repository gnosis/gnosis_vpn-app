...
Every PR opened against `main` gets the `backport release/hoprdv4` label
automatically (`.github/workflows/label-backport.yaml`). Keep it and
`.github/workflows/backport.yaml` cherry-picks the merged PR onto the stable
line, opening a PR that runs the same CI as any other. Remove the label before
