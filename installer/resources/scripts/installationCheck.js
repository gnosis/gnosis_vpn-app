...
if (system.version && system.version.ProductVersion) {
  const osVersion = system.version.ProductVersion;
...
if (system.sysctl) {
  ...
  const supportedArchitectures = ["x86_64", "arm64", "arm64e"];

  let isSupported = false;
  let i;
  for (i = 0; i < supportedArchitectures.length; i++) {
  ...
  if (!isSupported) {
    ...
    my.result.message = "Your system architecture (" + arch +
      ") is not supported. Only x86_64 (Intel) and arm64 (Apple Silicon) are supported.";
    my.result.type = "Fatal";
    false;
  }
}
